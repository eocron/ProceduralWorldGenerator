...
namespace ProceduralWorldGenerator.Common
{
    public static class ThemeManager
    {
        ...
        {
            if (!_themesResources.ContainsKey(themeName)) PreloadTheme(themeName);

            // Load new theme if it is valid
            if (_themesResources.TryGetValue(themeName, out var resources))
            {
                foreach (var res in resources) Application.Current.Resources.MergedDictionaries.Add(res);
        ...
        private static void PreloadTheme(string themeName)
        {
            if (!_themesUris.TryGetValue(themeName, out var preload))
            {
                preload = new List<Uri>(3)
                {
                    ...
                    new($"pack://application:,,,/ProceduralWorldGenerator.Common;component/Themes/{themeName}.xaml")
                };
            ...
            else if (ActiveTheme == null) ActiveTheme = themeName;
